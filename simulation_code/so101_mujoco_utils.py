...
_disengaged_steps = 0
_recent_contact_steps = 0
_recent_grasp_attempt_steps = 0
_post_attempt_commitment_steps = 0
...
def create_reward_state_tracker() -> dict[str, Any]:
    ...
    return {
        ...
        "recent_grasp_attempt_steps": 0,
        "post_attempt_commitment_steps": 0,
...
def _sync_global_reward_state(state: dict[str, Any]) -> None:
    ...
    _prev_near_contact = state["prev_near_contact"]
    ...
    _recent_contact_steps = state["recent_contact_steps"]
    _recent_grasp_attempt_steps = state["recent_grasp_attempt_steps"]
    _post_attempt_commitment_steps = state["post_attempt_commitment_steps"]
...
def compute_pickup_reward_from_state(
    ...
    recent_grasp_attempt_window: int = 10,
    post_attempt_commitment_window: int = 20,
    post_attempt_commitment_reward: float = 0.04,
    post_attempt_recovery_scale: float = 0.08,
    escape_posture_stall_threshold: int = 6,
    escape_posture_penalty: float = -0.06,
    post_attempt_escape_penalty: float = -0.10,
    min_pregrasp_gripper_qpos: float = 0.015,
    early_close_stall_threshold: int = 3,
    early_close_penalty: float = -0.05,
    ...
    prev_near_contact = state["prev_near_contact"]
    prev_gripper_qpos = state["prev_gripper_qpos"]
    prev_sustained = prev_contacted and state["consecutive_contact"] >= sustained_contact_threshold
    gripper_joint = float(d.qpos[5])
    gripper_closing = prev_gripper_qpos is not None and (prev_gripper_qpos - gripper_joint) > 0.002
    ...
    in_grasp_corridor = horizontal_dist < 0.08 and 0.006 < height_above < 0.06 and not contacted
    ...
    if not contacted:
        ...
        if moving_into_grasp:
            closeness_scale = approach_closeness_scale * (0.5 if (in_grasp_corridor or near_contact) else 1.0)
            approach_closeness_reward = closeness_scale * approach_closeness * (0.5 + 0.5 * vertical_alignment)
    ...
    pregrasp_zone = (in_grasp_corridor or near_contact) and not contacted
    gripper_open_enough = gripper_joint >= min_pregrasp_gripper_qpos
    premature_close_zone = not contacted and not pregrasp_zone
    timed_close_event = pregrasp_zone and gripper_closing

    if premature_close_zone and not gripper_open_enough:
        state["early_closed_gripper_steps"] += 1
    elif gripper_open_enough or contacted:
        state["early_closed_gripper_steps"] = 0
    else:
        state["early_closed_gripper_steps"] = max(0, state["early_closed_gripper_steps"] - 1)
    applied_early_close_penalty = (
        early_close_penalty
        if state["early_closed_gripper_steps"] >= early_close_stall_threshold
        else 0.0
    )
    reward += applied_early_close_penalty

    applied_approach_open_gripper_reward = 0.0
    if premature_close_zone and gripper_open_enough and (horizontal_progress > 0.0 or vertical_progress > 0.0):
        applied_approach_open_gripper_reward = approach_open_gripper_reward
        reward += applied_approach_open_gripper_reward

    applied_timed_close_reward = 0.0
    if timed_close_event:
        applied_timed_close_reward = timed_close_reward
        reward += applied_timed_close_reward
        state["timed_close_steps"] = timed_close_window
    else:
        state["timed_close_steps"] = max(0, state["timed_close_steps"] - 1)

    applied_gripper_closing_reward = 0.0
    grasp_attempt_event = timed_close_event
    if grasp_attempt_event:
    ...
    if pregrasp_not_committing:
        ...
        state["pregrasp_hover_steps"] = 0
    applied_pregrasp_hover_penalty = (
        pregrasp_hover_penalty
        if state["pregrasp_hover_steps"] >= pregrasp_hover_stall_threshold
        else 0.0
    )
    ...
    if contacted:
        ...
        if contact_entry:
            ...
            if state["recent_gripper_closing_steps"] > 0 or state["timed_close_steps"] > 0:
        ...
        if sustained and (gripped or block_height_gain > 0.01):
            ...
            reward += applied_sustained_contact_reward
        state["alignment_ready_steps"] = 0
    else:
    ...
    if contacted:
        ...
        state["recent_contact_steps"] = max(0, state["recent_contact_steps"] - 1)

    if grasp_attempt_event:
        state["recent_grasp_attempt_steps"] = recent_grasp_attempt_window
    else:
        state["recent_grasp_attempt_steps"] = max(0, state["recent_grasp_attempt_steps"] - 1)

    commitment_event = contacted or grasp_attempt_event or lift_progress_reward > 0.0
    if commitment_event:
        state["post_attempt_commitment_steps"] = max(
            state["post_attempt_commitment_steps"],
            post_attempt_commitment_window,
        )
    else:
        state["post_attempt_commitment_steps"] = max(0, state["post_attempt_commitment_steps"] - 1)

    commitment_active = state["post_attempt_commitment_steps"] > 0
    engaged_after_attempt = contacted or gripped or near_contact or in_grasp_corridor
    applied_post_attempt_commitment_reward = 0.0
    if commitment_active and engaged_after_attempt:
        recovery_reward = post_attempt_recovery_scale * np.clip(max(horizontal_progress, 0.0) / 0.02, 0.0, 1.0)
        applied_post_attempt_commitment_reward = post_attempt_commitment_reward + recovery_reward
        reward += applied_post_attempt_commitment_reward

    escape_candidate = (
        not contacted
        and not gripped
        and block_height_gain < 0.01
        and horizontal_dist > far_horizontal_threshold
        and horizontal_progress <= 0.0
    )
    if escape_candidate:
        state["escape_posture_steps"] += 1
    else:
        state["escape_posture_steps"] = max(0, state["escape_posture_steps"] - 1)
    applied_escape_posture_penalty = (
        escape_posture_penalty
        if state["escape_posture_steps"] >= escape_posture_stall_threshold
        else 0.0
    )
    applied_post_attempt_escape_penalty = (
        post_attempt_escape_penalty
        if applied_escape_posture_penalty and commitment_active
        else 0.0
    )
    ...
    if contacted and not gripped and block_height_gain < 0.01 and state["consecutive_contact"] >= contact_stall_threshold:
        applied_contact_stall_penalty = contact_stall_penalty
        reward += applied_contact_stall_penalty
    ...
    state["prev_near_contact"] = near_contact
    state["prev_block_height"] = block_pos[2]
    state["prev_gripper_qpos"] = gripper_joint
    ...
    metrics = {
        ...
        "post_attempt_commitment_reward": applied_post_attempt_commitment_reward,
        "post_attempt_commitment_steps": state["post_attempt_commitment_steps"],
        "recent_contact_steps": state["recent_contact_steps"],
        "recent_grasp_attempt_steps": state["recent_grasp_attempt_steps"],
        "escape_posture_penalty": applied_escape_posture_penalty,
        "post_attempt_escape_penalty": applied_post_attempt_escape_penalty,
        "escape_posture_steps": state["escape_posture_steps"],
        "early_close_penalty": applied_early_close_penalty,
        "early_closed_gripper_steps": state["early_closed_gripper_steps"],
        "approach_open_gripper_reward": applied_approach_open_gripper_reward,
        "timed_close_reward": applied_timed_close_reward,
        "timed_close_steps": state["timed_close_steps"],
        "gripper_open_enough": gripper_open_enough,
...
def compute_reward(
    ...
    recent_grasp_attempt_window=10,
    post_attempt_commitment_window=20,
    post_attempt_commitment_reward=0.04,
    post_attempt_recovery_scale=0.08,
    escape_posture_stall_threshold=6,
    escape_posture_penalty=-0.06,
    post_attempt_escape_penalty=-0.10,
    min_pregrasp_gripper_qpos=0.015,
    early_close_stall_threshold=3,
    early_close_penalty=-0.05,
    ...
    state = {
        ...
        "prev_near_contact": _prev_near_contact,
        ...
        "recent_contact_steps": _recent_contact_steps,
        "recent_grasp_attempt_steps": _recent_grasp_attempt_steps,
        "post_attempt_commitment_steps": _post_attempt_commitment_steps,
    ...
    reward, done, metrics = compute_pickup_reward_from_state(
        ...
        recent_grasp_attempt_window=recent_grasp_attempt_window,
        post_attempt_commitment_window=post_attempt_commitment_window,
        post_attempt_commitment_reward=post_attempt_commitment_reward,
        post_attempt_recovery_scale=post_attempt_recovery_scale,
        escape_posture_stall_threshold=escape_posture_stall_threshold,
        escape_posture_penalty=escape_posture_penalty,
        post_attempt_escape_penalty=post_attempt_escape_penalty,
        min_pregrasp_gripper_qpos=min_pregrasp_gripper_qpos,
        early_close_stall_threshold=early_close_stall_threshold,
        early_close_penalty=early_close_penalty,
    ...
    return (
        ...
        metrics["post_attempt_commitment_reward"],
        metrics["post_attempt_commitment_steps"],
        metrics["recent_contact_steps"],
        metrics["recent_grasp_attempt_steps"],
        metrics["escape_posture_penalty"],
        metrics["post_attempt_escape_penalty"],
        metrics["escape_posture_steps"],
        metrics["early_close_penalty"],
        metrics["early_closed_gripper_steps"],
        metrics["approach_open_gripper_reward"],
        metrics["timed_close_reward"],
        metrics["timed_close_steps"],
        metrics["gripper_open_enough"],
