...
The active bench adaptation lives in the repository's `src/so_arm101_v2` stack and `tools/`, using `model/bench_pick_replace_v1/scene_bench_pick_replace_v1.xml`. It is not launched by the legacy quick-start training commands below. Read the [bench runbook](../notes/bench-pick-replace-v1.md). The corrected square center is Y=0.2805353 m, derived from 8.5 inches beyond the base front edge. Joint map `measured_20260908b`, calibrated camera (fovy 44.0°) and detector `pad_normals_v2` are bound into `model/bench_pick_replace_v1/bench_config.json`; regenerate the scene only via `tools/prepare_bench_scene.py --bench-config`. The first bench training run (seed 202, 120k steps) launched 2026-09-08. Preserve existing legacy model files and results.
