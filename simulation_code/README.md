...
The active bench adaptation lives in the repository's `src/so_arm101_v2` stack and `tools/`, using `model/bench_pick_replace_v1/scene_bench_pick_replace_v1.xml`. It is not launched by the legacy quick-start training commands below. Read the [bench runbook](../notes/bench-pick-replace-v1.md). The corrected square center is Y=0.2805353 m, derived from 8.5 inches beyond the base front edge. Joint map `measured_20260908b`, the calibrated lens model (rational 8-coefficient, fovy 44.85°, rendered as a 90.34° pinhole at 1600×900 and resampled through the lens into the 256×256 full-frame observation) and detector `pad_normals_v2` are bound into `model/bench_pick_replace_v1/bench_config.json`; regenerate the scene only via `tools/prepare_bench_scene.py --bench-config [--intrinsics camera_intrinsics.json]`. The first bench training run (seed 202, 120k steps) ran 2026-09-08 on the pinhole scene (held-out 27/30); the lens-scene rerun on 2026-09-09 scored held-out 30/30. The physical runner (`tools/run_physical_episode.py`) drives this scene's policy on the real arm or, with `--sim-rehearsal`, on this scene. Preserve existing legacy model files and results.
