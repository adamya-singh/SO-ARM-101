...
**Placement randomization (2026-09-10, tranche implemented):** policies from the placement recipe expect the square (with the cube on it) anywhere inside a **14 × 10 in rectangle whose near edge is 2 in from the base front edge**, yawed up to ±45°. Tape that rectangle on the pad. The first 3 s chunk is now the survey move (shoulder up ~23°, elbow unfolds ~34°) to the viewing pose from which the camera sees the reachable rectangle; the real-frame gate (v3) checks that chunk against the simulated reference chunk; the placement reading at the reset pose is advisory (inside/outside the rectangle) and a second reading is taken from the survey frame at step 90. Unreachable for the teacher (so untrained): the far strip beyond ~9.5 in forward and the pocket closer than ~3.5 in within ~3.5 in of the centreline.

**Placement gate caveat (2026-09-10, episodes 11-13):** the back-projected cube position differs by ~25 mm between the reset pose and the episode end pose for the same cube (joint-map/camera bias), so the gate is advisory; `--no-cube-gate` records it without refusing. Success in episode 10 and the miss in episode 13 differed by ~10 mm of descent endpoint, so place the cube as close to the task pose as possible until the placement-randomized policy exists.
