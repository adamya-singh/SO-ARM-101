...
def main(argv=None) -> int:
    ...
    cap = cv2.VideoCapture(0, cv2.CAP_V4L2); cap.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc(*'MJPG')); cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1920); cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 1080)
    ...
    kept = []; t0 = time.time(); last = ''
    try:
        while time.time() - t0 < args.seconds and len(kept) < args.max_keep:
            before = np.array([[float(robot.bus.sync_read('Present_Position')[n]) for n in JOINT_NAMES] for _ in range(2)])
            for _ in range(3): ok, f = cap.read()
        ...
        cap.release(); disconnect_read_only(robot)
